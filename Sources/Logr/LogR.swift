    ...
    public func log(level: LogLevel,
        ...
        Task { [weak writer, cryptoService] in
            do {
                ...
                let encryptedLogEntry = EncryptedLogEntry(id: entry.id, timestamp: entry.timestamp, data: encryptedLogData)
...
//            limit: nil
//        ) ?? []
        return try format.encode(recentLogs)
    }
}
...
private extension LogR {
    private func loadRecentLogs() async {
        do {
            ...
            let logs: [LogEntry] = encryptedLogs?.compactMap { try? cryptoService.symmetricDecrypt(encryptedData: $0.data) } ?? []
//            let logs = try await storage?.retrieve(
//                levels: nil,
//                categories: nil,
//                subsystems: nil,
...
actor LogWriterActor {
    ...
    init(storage: LogRPersistence, configuration: LogrConfiguration) {
        self.storage = storage
        self.logger = Logger(subsystem: configuration.subsystem, category: LogCategory.persistence.rawValue)
    }
