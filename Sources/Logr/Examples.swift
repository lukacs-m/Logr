...
public enum LogRExamples {
    ...
    public static func filteringLogs() async throws {
        let logger = await LogR.shared
        
        // Add some test logs
        await logger.info("User action", category: "ui")
        await logger.error("Network error", category: "networking")
        await logger.debug("Debug info", category: "development")
        
        // Filter by log level
        let errorLogs = try await logger.getLogs(levels: [.error, .fault])
        print("Found \(errorLogs.count) error/fault logs")
        
        // Filter by category
        let networkLogs = try await logger.getLogs(categories: ["networking"])
...
public struct LogRSwiftUIExamples {
    ...
    public struct LoggingControlsView: View {
        @State private var logger = LogR.shared
        ...
        struct ContentView: View {
            var body: some View {
                VStack {
                    ...
                    Button("Trigger Log") {
                        Task {
                            await LogR.shared.info("Button tapped", category: "ui")
                        }
                    }
                }
                .navigationTitle("App")
